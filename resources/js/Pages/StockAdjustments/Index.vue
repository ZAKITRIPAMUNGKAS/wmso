...
const deleteItem = (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus penyesuaian stok ini? Pilihan ini hanya tersedia untuk draf.')) {
    ...
    <div>
        ...
        <AuthenticatedLayout title="Stock Opname">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 font-sans">
                <div>
                    <h2 class="text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Penyesuaian Stok (Stock Opname)</h2>
                    <p class="text-[11px] text-slate-500 font-bold mt-1 uppercase tracking-tight">Relevansi Stok Fisik vs Stok Sistem</p>
                </div>
                <Link v-if="$page.props.auth.user.role !== 'viewer'" :href="route('stock-adjustments.create')" class="btn-primary flex items-center justify-center gap-2">
                    <PhPlus weight="bold" /> 
                    <span class="tracking-tight uppercase font-black text-[10px]">Opname Baru</span>
                </Link>
            </div>

            <!-- Table Section -->
            <ResponsiveTable :headers="['No. Opname', 'Gudang', 'Tanggal', 'Pembuat', 'Status', 'Catatan']" :items="adjustments.data">
                <template #row="{ item }">
                    <td class="px-8 py-5">
                    ...
                    <td class="px-8 py-5 text-sm font-bold text-slate-500 max-w-[200px] truncate">
                        {{ item.catatan || '-' }}
                    </td>
                    <td class="px-8 py-5 text-right">
                        <div class="flex justify-end gap-2">
                            <!-- View Details -->
                            <Link :href="route('stock-adjustments.show', item.id)" class="p-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 transition shadow-sm active:scale-95">
                                <PhEye :size="18" weight="bold" />
                            </Link>
                            
                            <!-- Edit (Draft status only and not viewer) -->
                            <Link v-if="$page.props.auth.user.role !== 'viewer' && item.status === 'draft'" :href="route('stock-adjustments.edit', item.id)" class="p-2 bg-amber-50 text-amber-600 rounded-xl hover:bg-amber-100 transition shadow-sm active:scale-95">
                ...
                <template #mobile-card="{ item }">
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600">
                                <PhClipboardText :size="20" weight="fill" />
                            </div>
                            <div>
                                <div class="font-black text-slate-800 tracking-tight uppercase">{{ item.no_adjustment }}</div>
                        ...
                        <div class="flex flex-col items-end gap-2">
                            ...
                            <div class="flex gap-1.5 mt-1">
                                <Link :href="route('stock-adjustments.show', item.id)" class="p-2 text-indigo-600 bg-indigo-50 rounded-lg">
                                    <PhEye :size="16" weight="bold" />
                                </Link>
                                <Link v-if="$page.props.auth.user.role !== 'viewer' && item.status === 'draft'" :href="route('stock-adjustments.edit', item.id)" class="p-2 text-amber-600 bg-amber-50 rounded-lg">
                    ...
                    <!-- Warehouse & User info on Mobile -->
                    <div class="border-t border-slate-50 pt-4 mt-2 space-y-2">
                        <div class="flex items-center gap-1.5 text-[11px] font-black text-slate-600 tracking-tight">
                            <PhWarehouse :size="12" class="text-slate-400" />
                            <span class="uppercase">Gudang: {{ item.warehouse?.nama }}</span>
                        </div>
                        <div class="flex items-center gap-1.5 text-[10px] text-slate-400 font-bold uppercase mt-1">
                            <PhUser :size="12" />
                            <span>{{ item.user?.name }}</span>
                        </div>
                        <div v-if="item.catatan" class="text-[10px] text-slate-400 font-bold uppercase mt-1">
                ...
                <template #pagination>
                    <div class="flex justify-between items-center w-full px-6 py-4 bg-slate-50/50 uppercase">
                        <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest italic">
                            Data {{ adjustments.from || 0 }}-{{ adjustments.to || 0 }} dari {{ adjustments.total }}
                        </p>
