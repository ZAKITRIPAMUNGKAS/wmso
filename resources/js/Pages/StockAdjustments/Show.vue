...
const getOfficialNumber = computed(() => {
    return props.adjustment?.no_adjustment || '-';
});
...
const totalItems = computed(() => props.adjustment?.items?.length || 0);

const totalSistem = computed(() => {
    return props.adjustment?.items?.reduce((acc, item) => acc + (parseInt(item.quantity_sistem) || 0), 0);
});

const totalFisik = computed(() => {
    return props.adjustment?.items?.reduce((acc, item) => acc + (parseInt(item.quantity_fisik) || 0), 0);
});

const totalSelisih = computed(() => {
    return props.adjustment?.items?.reduce((acc, item) => acc + (parseInt(item.selisih) || 0), 0);
});
...
const confirmAdjustment = () => {
    if (confirm('Apakah Anda yakin ingin menyelesaikan penyesuaian stok ini? Tindakan ini akan langsung mensinkronisasikan stok sistem dan fisik di gudang terpilih.')) {
        router.put(route('stock-adjustments.update', props.adjustment.id), {
            warehouse_id: props.adjustment.warehouse_id,
            tanggal: props.adjustment.tanggal ? props.adjustment.tanggal.substring(0, 10) : '',
            status: 'completed',
            catatan: props.adjustment.catatan,
            items: props.adjustment.items.map(i => ({
                product_id: i.product_id,
                quantity_sistem: i.quantity_sistem,
                quantity_fisik: i.quantity_fisik
            }))
        });
    }
};
...
const settings = reactive({
    ...
    docDate: formatDate(props.adjustment.tanggal),
    warehouse: props.adjustment.warehouse?.nama.toUpperCase(),
    senderSignatory: props.adjustment.user.name.toUpperCase(), // Disiapkan Oleh
    receiverSignatory: '', // Pemeriksa / Auditor
    managerSignatory: '', // Mengetahui
    catatan: props.adjustment.catatan || 'Kuantitas fisik telah dihitung secara langsung dan dicocokkan dengan kuantitas sistem. Selisih yang terjadi telah direkonsiliasi dalam sistem WMS.'
});
</script>

<template>
    <div>
        <Head :title="adjustment.no_adjustment" />

        <AuthenticatedLayout :title="'Dokumen ' + adjustment.no_adjustment">
            ...
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 no-print font-sans">
                <Link :href="route('stock-adjustments.index')" class="text-[11px] font-bold text-slate-500 hover:text-[#1E3A5F] flex items-center gap-2 uppercase tracking-wider transition">
                    <PhArrowLeft weight="bold" /> Kembali ke Index
                </Link>
                <div class="flex flex-wrap items-center gap-3">
                    <button v-if="$page.props.auth.user.role !== 'viewer' && adjustment.status === 'draft'" @click="confirmAdjustment" class="bg-emerald-600 text-white px-6 py-2 text-[11px] font-bold uppercase tracking-widest hover:bg-emerald-700 transition shadow-sm active:scale-95 flex items-center gap-2">
                        <PhCheckCircle weight="bold" :size="16" /> Konfirmasi Penyesuaian
                    </button>
                    ...
                    <div class="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide font-sans">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-1.5">
                                <label class="text-[9px] font-black text-slate-400 uppercase">Nomor Opname (ADJ)</label>
                            ...
                            <div class="space-y-1.5">
                                <label class="text-[9px] font-black text-slate-400 uppercase">Gudang</label>
                                <input v-model="settings.warehouse" type="text" class="w-full bg-slate-50 border-none rounded-lg text-xs font-bold text-slate-700 focus:ring-2 focus:ring-indigo-600/20">
                            </div>
                        </div>
                        ...
                        <div class="space-y-4 pt-4 border-t border-slate-100">
                            ...
                            <div class="space-y-3">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-black text-slate-400 uppercase">Disiapkan Oleh (Kiri)</label>
                                ...
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-black text-slate-400 uppercase">Pemeriksa / Auditor (Tengah)</label>
                                    <input v-model="settings.receiverSignatory" type="text" placeholder="Nama Auditor / Checker" class="w-full bg-slate-50 border-none rounded-lg text-xs font-bold text-slate-700 focus:ring-2 focus:ring-indigo-600/20">
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-black text-slate-400 uppercase">Mengetahui (Kanan)</label>
                                    <input v-model="settings.managerSignatory" type="text" placeholder="Nama Supervisor / Manager" class="w-full bg-slate-50 border-none rounded-lg text-xs font-bold text-slate-700 focus:ring-2 focus:ring-indigo-600/20">
                                </div>
                            </div>
                        </div>
            ...
            <!-- OFFICIAL CORPORATE DOCUMENT CONTAINER (ADJ) -->
            <div class="print-container bg-white border border-[#D1D5DB] shadow-sm relative font-business text-[#1F2937] leading-tight mx-auto overflow-hidden">
                ...
                <div class="absolute top-4 right-8 text-[8px] italic font-black text-slate-300 uppercase tracking-[0.2em] print-only">
                    Lembar 1 - ASLI (Arsip Logistik)
                </div>

                <!-- 1. KOP SURAT SURAT PENYESUAIAN -->
                ...
                <div class="p-8">
                    ...
                    <div class="mb-6 flex justify-between items-start text-[10px] font-sans">
                        <div class="space-y-1">
                            <p><span class="font-black w-28 inline-block">PERIHAL</span> : Berita Acara Penyesuaian Fisik & Sistem (Stock Opname)</p>
                            <p><span class="font-black w-28 inline-block">GUDANG</span> : {{ settings.warehouse }}</p>
                            <p><span class="font-black w-28 inline-block">OPERATOR</span> : {{ adjustment.user.name.toUpperCase() }}</p>
                        </div>
                        <div class="text-right space-y-1">
                            <p><span class="font-black italic tracking-tighter">Jakarta,</span> {{ settings.docDate }}</p>
                            <div class="inline-block border-2 border-double px-3 py-1 mt-2 transform -rotate-2" :class="[adjustment.status === 'completed' ? 'border-[#166534]' : 'border-amber-600']">
                                <span class="text-[11px] font-serif font-black uppercase tracking-[0.2em]" :class="[adjustment.status === 'completed' ? 'text-[#166534]' : 'text-amber-600']">
                                    {{ adjustment.status === 'completed' ? 'OPNAME SELESAI' : 'OPNAME DRAF' }}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-[#D1D5DB] pt-4 mb-4">
                        <p class="text-[10px] italic font-serif text-[#4B5563] mb-4">
                            Telah dilaksanakan perhitungan fisik persediaan barang secara komprehensif dengan hasil perbandingan terhadap stok sistem sebagai berikut:
                        </p>
                    </div>
                    ...
                    <table class="w-full border-collapse border border-[#1F2937] mb-6 font-sans">
                        <thead>
                            <tr class="bg-[#1E3A5F] text-white">
                                ...
                                <th class="border border-[#1F2937] p-2 text-[10px] font-serif font-black uppercase text-left w-[40%]">NAMA PRODUK / SPESIFIKASI</th>
                                <th class="border border-[#1F2937] p-2 text-[10px] font-serif font-black uppercase text-center w-[13%]">STOK SISTEM</th>
                                <th class="border border-[#1F2937] p-2 text-[10px] font-serif font-black uppercase text-center w-[13%]">STOK FISIK</th>
                                <th class="border border-[#1F2937] p-2 text-[10px] font-serif font-black uppercase text-center w-[12%]">SELISIH</th>
                                <th class="border border-[#1F2937] p-2 text-[10px] font-serif font-black uppercase text-center w-[12%]">SATUAN</th>
                                <th class="border border-[#1F2937] p-2 text-[10px] font-serif font-black uppercase text-center w-[10%]">KET</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(item, index) in adjustment.items" :key="item.id" class="page-break-avoid h-10">
                                ...
                                <td class="border border-[#1F2937] p-2 text-[11px] font-black text-center text-slate-500">{{ item.quantity_sistem }}</td>
                                <td class="border border-[#1F2937] p-2 text-[11px] font-black text-center">{{ item.quantity_fisik }}</td>
                                <td :class="[
                                    'border border-[#1F2937] p-2 text-[11px] font-black text-center',
                                    item.selisih > 0 ? 'text-emerald-700' : (item.selisih < 0 ? 'text-rose-700' : 'text-slate-600')
                                ]">
                                ...
                                <td class="border border-[#1F2937] p-2 text-[10px] text-center font-bold">
                                    {{ item.selisih === 0 ? 'COCOK' : 'REKONSILIASI' }}
                                </td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr class="bg-slate-50">
                                <td colspan="2" class="border border-[#1F2937] p-2 text-[10px] font-serif font-black uppercase text-right">TOTAL AKUMULASI</td>
                                <td class="border border-[#1F2937] p-2 text-[11px] font-black text-center text-slate-500">{{ totalSistem }}</td>
                                <td class="border border-[#1F2937] p-2 text-[11px] font-black text-center">{{ totalFisik }}</td>
                                <td :class="[
                                    'border border-[#1F2937] p-2 text-[11px] font-black text-center',
                                    totalSelisih > 0 ? 'text-emerald-700' : (totalSelisih < 0 ? 'text-rose-700' : 'text-slate-600')
                                ]">
                                    {{ totalSelisih > 0 ? '+' + totalSelisih : totalSelisih }}
                                </td>
                                <td colspan="2" class="border border-[#1F2937] p-2 text-[10px] text-center font-black uppercase">{{ totalItems }} SKU</td>
                            </tr>
                        </tfoot>
                    </table>
                    ...
                    <div class="grid grid-cols-12 gap-8 mb-12 font-sans">
                        <div class="col-span-8">
                            <div class="border border-[#D1D5DB] p-4 min-h-[60px] relative">
                                <p class="absolute -top-2 left-3 bg-white px-2 text-[9px] font-black uppercase tracking-widest text-[#4B5563]">Catatan / Keterangan Penyesuaian</p>
                        ...
                        <div class="col-span-4 flex flex-col items-center justify-center border border-[#D1D5DB] bg-slate-50/50 p-3">
                            ...
                            <p class="text-[7px] font-sans font-black uppercase mt-2 tracking-widest text-slate-500 text-center">Verifikasi Opname Digital</p>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-start font-serif mb-12 px-10">
                        <div class="text-center w-[200px]">
                            <p class="text-[11px] font-black mb-16 uppercase">Disiapkan Oleh,</p>
                            ...
                            <p class="text-[9px] font-sans font-bold text-slate-400 uppercase">Operator / Gudang</p>
                        </div>
                        
                        <div class="text-center w-[200px]">
                            <p class="text-[11px] font-black mb-16 uppercase italic">Pemeriksa / Checker,</p>
                            <div class="border-b-[1.5px] border-[#1F2937] w-full"></div>
                            <p class="text-[10px] font-black uppercase mt-1 italic tracking-widest">{{ settings.receiverSignatory || '( ............................ )' }}</p>
                            <p class="text-[9px] font-sans font-bold text-slate-400 uppercase tracking-widest">Internal Auditor</p>
                        </div>
                        ...
                        <div class="text-center w-[200px]">
                            ...
                            <p class="text-[9px] font-sans font-bold text-slate-400 uppercase">Supervisor / Manager</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 pt-2 border-t-[3px] border-double border-[#1F2937] bg-slate-50/50 print-footer">
                    <p class="text-center text-[8px] italic font-serif text-[#6B7280] mb-2 leading-none">
                        "Dokumen ini merupakan berita acara resmi penyesuaian inventaris gudang internal yang diterbitkan secara sah oleh sistem WMS CV. Listrindo Jaya Elektrik."
                    </p>
                    <div class="flex justify-between items-center text-[8px] font-sans font-black text-[#9CA3AF] uppercase tracking-widest border-t border-slate-200 pt-2 px-4">
                        <span>ADJ-ID: {{ settings.docNumber }}</span>
